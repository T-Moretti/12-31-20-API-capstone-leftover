...
function insertInstructions(id, responseJson) {
    ...
    let instructionsArray = responseJson; //problem might lie here
    $(`#${id}`).append(`<h3>Instructions</h3><ul>`)
    for (let i = 0; i < instructionsArray[0].steps.length; i++) {
        $(`#${id}`).append(`<li>${instructionsArray[0].steps[i].step}</li>`) //having an issue just pulling  object,object,object
    }
    $(`#${id}`).append(`</ul>`)
};
...
// currently displays all results - There are two other functions called at the 
// bottom which uses two other API's to fetch data
